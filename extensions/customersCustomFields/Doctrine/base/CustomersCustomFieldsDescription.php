...
class CustomersCustomFieldsDescription extends Doctrine_Record {
	...
	public function setTableDefinition(){
		$this->setTableName('customers_custom_fields_description');
	...
	public function newLanguageProcess($fromLangId, $toLangId){
		...
		->from('CustomersCustomFieldsDescription')
		...
		foreach($Qdescription as $Record){
			...
			EventManager::notify('CustomersCustomFieldsDescriptionNewLanguageProcessBeforeTranslate', $toTranslate);
			
			$translated = sysLanguage::translateText($toTranslate, (int) $toLangId, (int) $fromLangId);
			
			$newDesc = new CustomersCustomFieldsDescription();
			$newDesc->field_id = $Record['field_id'];
			$newDesc->language_id = (int) $toLangId;
			$newDesc->field_name = $translated['name'];
			
			EventManager::notify('CustomersCustomFieldsDescriptionNewLanguageProcessBeforeSave', $newDesc);
	...
	public function cleanLanguageProcess($existsId){
		...
		->delete('CustomersCustomFieldsDescription')
