<?php
	$CustomersCustomFieldsGroups = Doctrine_Core::getTable('CustomersCustomFieldsGroups');
	if (isset($_GET['gID'])){
		$Group = $CustomersCustomFieldsGroups->find((int)$_GET['gID']);
	}else{
		$Group = $CustomersCustomFieldsGroups->create();
	}
	...
	if (isset($_GET['gID'])){
		...
		$iconCss = array(
 		...
 		->setHref(itw_app_link('appExt=customersCustomFields&action=removeGroup&group_id=' . $Group->group_id))
 		->css($iconCss);

 		$editIcon = htmlBase::newElement('icon')->setType('wrench')->setTooltip('Click to edit group')
 		->setHref(itw_app_link('appExt=customersCustomFields&action=getGroupWindow&group_id=' . $Group->group_id))
 		->css($iconCss);
