...
class productDesigner_admin_products_new_product extends Extension_productDesigner {
	...
	public function NewProductAddTabs(Product $Product, $ProductType, htmlWidget_tabs &$Tabs) {
		if ($ProductType->getCode() == 'standard'){
			$Tabs
				->addTabHeader('tab_' . $this->getExtensionKey(), array('text' => sysLanguage::get('TAB_PRODUCT_DESIGNER')))
	...
	public function buildEditableAreasTab(&$tabObj, Product &$Product){
		$productImageEditable = htmlBase::newElement('img')->addClass('productImageEditable')->attr('src', sysConfig::getDirWsCatalog() . sysConfig::get('DIR_WS_IMAGES') . $Product->getImage());
		$productImageEditableBack = htmlBase::newElement('img')->addClass('productImageEditableBack')->attr('src', sysConfig::getDirWsCatalog() . sysConfig::get('DIR_WS_IMAGES') . $Product->getImageBack());
