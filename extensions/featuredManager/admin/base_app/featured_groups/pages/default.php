<?php
	...
	->select('g.*')
	->from('FeaturedManagerGroups g')
	->orderBy('g.featured_group_name');
	...
	if ($groups){
		foreach($groups as $group){
			$groupId = $group['featured_group_id'];
			$rows++;
			...
			$tableGrid->addBodyRow(array(
				...
				'columns' => array(
					array('text' => $folderIcon->draw() . $group['featured_group_name']),
	...
	switch ($action) {
		case 'delete_group':
			...
			$infoBox->addContentRow(sysLanguage::get('TEXT_DELETE_CATEGORY_INTRO') . tep_draw_hidden_field('featured_group_id', $current_group['featured_group_id'] ));
			$infoBox->addContentRow('<b>' . $current_group['featured_group_name'] . '</b>');

			break;
		default:
			$infoBox->setButtonBarLocation('top');
			if (tep_not_null($current_group)) {
				$groupName = $current_group['featured_group_name'];
				$groupId = $current_group['featured_group_id'];
