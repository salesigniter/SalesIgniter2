...
if ($Widget !== false){
	...
	$TemplatePaths = $LayoutBuilder->getWidgetTemplatePaths($_GET['widgetCode']);
	foreach($TemplatePaths as $WidgetTemplateCode => $WidgetTemplatePath){
	...
	if (file_exists($Widget->getPath() . 'windows/editWidget.php')){
		require($Widget->getPath() . 'windows/editWidget.php');
	}

	$saveButton = htmlBase::newElement('button')
		->attr('data-widget_code', $Widget->getCode())
		->addClass('saveButton')
		->usePreset('save');
