...
class TemplateManagerWidgetBase extends ModuleBase
{
	...
	public function init($moduleCode, $forceEnable = false, $moduleDir = false) {
		global $App;
		if ($moduleDir === false){
			$moduleDir = sysConfig::getDirFsCatalog() . 'extensions/templateManager/' . $this->widgetDir . '/' . $moduleCode;
		}
		parent::init($moduleCode, false, $moduleDir);

		$this->boxTemplateDefaultDir = sysConfig::getDirFsCatalog() . 'extensions/templateManager/' . $this->widgetTemplateDir . '/';
	}
