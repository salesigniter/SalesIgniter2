...
class TemplateManagerWidgetShoppingCart extends TemplateManagerWidget
{
	...
	{
		global $App;
		$this->init('shoppingCart', false, __DIR__);
		...
		if ($userAccount->isLoggedIn() === true){
			...
			if ($voucherAvailableAmount > 0){
				...
				$voucherBalance->addBodyRow(array(
					'columns' => array(
						array('addCls' => 'main', 'text' => sysLanguage::get('WIDGET_SHOPPINGCART_VOUCHER_BALANCE')),
		...
		if (Session::exists('gv_id') === true){
			...
			if ($Qcoupon){
				...
				$couponAmount->addBodyRow(array(
					'columns' => array(
						array('addCls' => 'main', 'text' => sysLanguage::get('WIDGET_SHOPPINGCART_VOUCHER_REDEEMED')),
