...
class TemplateManagerWidgetSignupBox extends TemplateManagerWidget
{
	...
	{
		global $App;
		$this->init('signupBox', false, __DIR__);
	}
	...
	public function show(TemplateManagerLayoutBuilder $LayoutBuilder)
	{
		...
		if (!in_array($App->getAppName(), $dontShow) && $userAccount->isLoggedIn() === false){
			if ($userAccount->isLoggedIn() === false){
				...
				$loginTable->addBodyRow(array('columns' => array(array('text' => sysLanguage::get('WIDGET_LOGINBOX_EMAIL')))));
				...
				$loginTable->addBodyRow(array('columns' => array(array('text' => sysLanguage::get('WIDGET_LOGINBOX_RE_EMAIL')))));
				$loginTable->addBodyRow(array('columns' => array(array('text' => $loginEmailAddress1))));
				$loginPassword = htmlBase::newElement('input')
					->css('width', '90%')
					->setType('password')
					->setName('password1');
				$loginTable->addBodyRow(array('columns' => array(array('text' => sysLanguage::get('WIDGET_LOGINBOX_PASSWORD')))));
				$loginTable->addBodyRow(array('columns' => array(array('text' => $loginPassword))));
				$loginPassword1 = htmlBase::newElement('input')
					->css('width', '90%')
					->setType('password')
					->setName('password2');
				$loginTable->addBodyRow(array('columns' => array(array('text' => sysLanguage::get('WIDGET_LOGINBOX_RE_PASSWORD')))));
