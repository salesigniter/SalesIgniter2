...
class TemplateManagerWidget extends ModuleBase
{
	...
	public function init($moduleCode, $forceEnable = false, $moduleDir = false) {
		global $App;
		if ($moduleDir === false){
			$moduleDir = sysConfig::getDirFsCatalog() . 'extensions/templateManager/widgets/' . $moduleCode;
		}
		parent::init($moduleCode, false, $moduleDir);

		$this->setBoxHeading(sysLanguage::get($this->getModuleInfo('title_key')));
		$this->boxTemplateDefaultDir = sysConfig::getDirFsCatalog() . 'extensions/templateManager/widgets_templates/';
	}
