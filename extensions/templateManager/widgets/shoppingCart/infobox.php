...
class InfoBoxShoppingCart extends InfoBoxAbstract {
	...
	public function showLayoutPreview($WidgetSettings){
		...
		if ($WidgetSettings->box_mode == 'mini'){
			$return = 'X ' . sysLanguage::get('INFOBOX_SHOPPINGCART_ITEMS_IN_CART');
		}
		return $return;
	}
	...
	public function show(){
		...
		$boxMode = 'full';
		if (isset($WidgetProperties->box_mode)){
			$boxMode = $WidgetProperties->box_mode;
		}

		if ($boxMode == 'mini'){
			$htmlContent = $ShoppingCart->countContents() . sprintf(sysLanguage::get('INFOBOX_SHOPPINGCART_ITEMS_IN_CART'), itw_app_link(null, 'shoppingCart', 'default'));
		}else{
