<?php
	...
	if ($myReviews){
		foreach($myReviews as $reviews){
			...
			$productImage = '<img src="imagick_thumb.php?width=150&height=150&path=rel&imgSrc='.  $reviews['Products'][0]['products_image'].'" alt="'.$productName.'"/>';
			$reviewText = nl2br(tep_output_string_protected($reviews['ReviewsDescription'][0]['reviews_text']));
			$reviewRating = sprintf(susLanguage::get('TEXT_REVIEW_RATING'), tep_image(sysConfig::get('DIR_WS_TEMPLATE_IMAGES') . 'stars_' . $reviews['reviews_rating'] . '.gif', sprintf(sysLanguage::get('TEXT_OF_5_STARS'), $reviews['reviews_rating'])), sprintf(sysLanguage::get('TEXT_OF_5_STARS'), $reviews['reviews_rating']));
