...
class TemplateManagerWidgetReview extends TemplateManagerWidget {
	...
	public function show(TemplateManagerLayoutBuilder $LayoutBuilder){
			...
			if($myReviews){
				foreach($myReviews as $review){
					...
					$boxContent = '<div align="center" class="reviewbox"><a href="' . itw_app_link('appExt=reviews&products_id=' . $review['products_id'],'product_review','default')  . '"><img src="imagick_thumb.php?width=150&height=150&path=rel&imgSrc=' .  $review['Products'][0]['products_image'] . '"></a><br /><a href="' . itw_app_link('appExt=reviews&products_id=' . $review['products_id'],'product_review','default') . '">' . $review1 . '</a><br />' . tep_image(sysConfig::get('DIR_WS_TEMPLATE_IMAGES') . 'stars_' . $review['reviews_rating'] . '.gif' , sprintf(sysLanguage::get('INFOBOX_REVIEWS_TEXT_OF_5_STARS'), $review['reviews_rating'])) . '</div>';
				}
			} elseif (isset($_GET['products_id'])) {
				$boxContent = '<table border="0" cellspacing="0" cellpadding="2"><tr><td class="infoBoxContents"><a href="' . itw_app_link('appExt=reviews&products_id=' . $_GET['products_id'], 'product_review', 'write') . '">' . tep_image(sysConfig::get('DIR_WS_TEMPLATE_IMAGES') . 'box_write_review.gif', sysLanguage::get('IMAGE_BUTTON_WRITE_REVIEW')) . '</a></td><td class="infoBoxContents"><a href="' . itw_app_link('appExt=reviews&products_id=' . $review['products_id'],'product_review','write') . '">' . sysLanguage::get('INFOBOX_REVIEWS_WRITE_REVIEW') .'</a></td></tr></table>';
			} else {
