...
$PackageBox = htmlBase::newElement('infobox')
	->setForm(array(
	'name' => 'cart_quantity',
	'action' => itw_app_link('action=addCartProduct&pID=' . $Product->getId(), 'product', 'info')
))
	->css('width', 'auto')->removeCss('margin-left')->removeCss('margin-right')
	->setHeader('Package\'s Products')
	->setButtonBarLocation('bottom');

if ($ProductType->hasInventory() === true){
	$PackageBox->addButton(htmlBase::newElement('button')->setType('submit')->setText('Add To Cart'));
}else{
	$PackageBox->addButton(htmlBase::newElement('button')->setType('submit')->setText('Add To Cart')->disable());
}
$PackageBox->addButton(htmlBase::newElement('input')->setType('hidden')->setName('products_id')->val($Product->getId()));

if ($ProductType->hasReservation()){
	$resProducts = array();
	$resPurchaseTypes = array();
	foreach($ProductType->getProductsRaw() as $PackageInfo){
		$PackageProduct = $PackageInfo['productClass'];
		$PackageData = $PackageInfo['packageData'];
		if (isset($PackageData->purchase_type) && $PackageData->purchase_type == 'reservation'){
			$resProducts[] = $PackageProduct->getId();
			$resPurchaseTypes[] = $PackageProduct->getProductTypeClass()->getPurchaseType('reservation');
		}
	}
	/*
	 * Needs a way to hide the quantity box
	 * $resQty argument needs to be array to handle multiple products quantities in package
	 */
	$PackageBox->addContentRow(ReservationUtilities::getCalendar(
		$resProducts,
		$resPurchaseTypes,
		1,/*Quantity needs to be array to handle multiple products quantities in package*/
		true,/*Show shipping selection*/
		'catalog',/*Calling From Catalog*/
		array(), /*Use All Barcodes*/
		false /*Dont show button*/
	));
}
foreach($ProductType->getProductsRaw() as $PackageInfo){
	$PackageProduct = $PackageInfo['productClass'];
	...
	$PackageProductTypeClass = $PackageProduct->getProductTypeClass();
	if (isset($PackageData->purchase_type)){
		$PackageProductPurchaseType = $PackageProductTypeClass->getPurchaseType($PackageData->purchase_type);
	}

	if (isset($PackageData->price)){
		$price = $PackageData->price;
	}else{
		if (isset($PackageProductPurchaseType)){
			$price = $PackageProductPurchaseType->getPrice();
		}else{
			$price = $PackageProductTypeClass->getPrice();
		}
	}

	if (isset($PackageProductPurchaseType)){
		$type = $PackageProductPurchaseType->getTitle();
	}else{
		$type = $PackageProductTypeClass->getTitle();
	}

	$PackageBox->addContentRow($PackageData->quantity . 'x ( ' . $type . ' ) ' . $PackageProduct->getName());
}
$PackageBox->addContentRow('<hr>');
$PackageBox->addContentRow('<div style="text-align:right;font-weight:bold;font-size:1.3em;">' . $currencies->format($Product->getPrice()) . '</div>');

echo $PackageBox->draw();
/*
