...
$OrderProduct = $Editor->ProductManager->get((int) $_POST['id']);
    ...
    if (sysConfig::get('EXTENSION_PAY_PER_RENTALS_USE_EVENTS') == 'False'){
		...
		if($Qevent){
			...
			$bookings = $PurchaseType->getBookedDaysArray(date('Y-m-d', strtotime($start_date)), $rQty, &$reservArr, &$barcodesBooked, $Editor->ProductManager->getContents());
	...
	if (isset($resInfo['start_date']) && isset($resInfo['end_date'])){
		...
		$PurchaseType->processAddToOrderOrCart($resInfo, $reservationInfo);
		...
		//}
		$OrderProduct->setInfo($reservationInfo);
