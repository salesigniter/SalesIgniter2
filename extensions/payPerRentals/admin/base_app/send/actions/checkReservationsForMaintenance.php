...
foreach($QMaintenancePeriod as $mPeriod){
	...
	foreach($Qmaint as $bhr){
		if ($bhr['ProductsInventoryBarcodes']['BarcodeHistoryRented']['current_maintenance_type'] != 0) {
			continue;
		}
		$ProductBarcode = Doctrine_Core::getTable('ProductsInventoryBarcodes')
		->find($bhr['barcode_id']);
		...
		->find($ProductBarcode->barcode_id);
		if (!$barcodeHistory){
