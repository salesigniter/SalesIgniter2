...
function popupWindowEditReservation(type, rID, barcode_id, selectedBarcodes, products_id, w, h) {
    ...
    function overTd(){
        var $self = $(this);
			$self.data('toolTip', null);
			$self.data('toolTip2', null);
			$self.data('request', null);
       					this.style.cursor = 'pointer';
                        if ($(this).attr('isEdit') == '0'){
                            var offSet = $(this).offset();
                            Tooltip = showToolTipAttr({
                            ...
                            var offTop = offSet2.top - $(this).height() - 10;
                            ...
                            $request = $.ajax({
                                cache: false,
                                url: $('#editWindowForm').attr('action') + '?action=getResInfo' + '&type=' + type + '&rID=' + rID,
                                data:'',
                                type: 'post',
                                dataType: 'json',
                                success: function (data){
                                    if (data.success == true){
                                        if ($self.data('toolTip2') != null){
                                            Tooltip2.html(data.tooltip)
                                            .css({
                                                position: 'absolute',
                                                top: offTop - 150,
                                                zIndex: 9999,
    ...
    function outTd(){
        var $self = $(this);
          this.style.cursor = 'default';
                        if ($self.data('request') != null){
				        	$self.data('request').abort();
                            $self.data('request', null);
				        }

                        if ($self.data('toolTip') != null){
                            $self.data('toolTip').remove();
                            $self.data('toolTip', null);
                        }
                        if ($self.data('toolTip2') != null){
                            $self.data('toolTip2').remove();
                            $self.data('toolTip2', null);
                        }
    }
