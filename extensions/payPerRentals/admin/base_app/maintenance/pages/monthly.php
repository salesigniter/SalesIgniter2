<?php
	...
	->orWhere('bhr.last_monthly_date is null AND DATE_SUB(NOW(), INTERVAL 1 MONTH)<='.sysConfig::get('EXTENSION_PAY_PER_RENTALS_START_MAINTENANCE'))
	->orWhere('DATE_SUB(NOW(), INTERVAL 1 MONTH) <= bhr.last_monthly_date');
	...
	switch ($action){
		case 'edit':
			$infoBox->setForm(array(
					'action'    => itw_app_link(tep_get_all_get_params(array('action')) . 'action=save&type=3'),
					'method'    =>  'post',
			...
			$cancelButton = htmlBase::newElement('button')
				->usePreset('cancel')
				->setHref(itw_app_link(tep_get_all_get_params(array('action', 'appPage')), null, 'monthly', 'SSL'));
		...
		default:
			if (isset($cInfo) && is_object($cInfo)) {
				...
				->setHref(itw_app_link(tep_get_all_get_params(array('action', 'mID')) . 'action=edit&mID=' . $cInfo->pay_per_rental_maintenance_id,'maintenance','monthly'));
...
?>
 <div class="pageHeading"><?php echo sysLanguage::get('HEADING_TITLE_MONTHLY');?></div>
 <br />
