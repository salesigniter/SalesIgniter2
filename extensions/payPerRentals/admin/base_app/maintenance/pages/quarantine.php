<?php
	...
	->where('bhr.number_rents >= ?',sysConfig::get('EXTENSION_PAY_PER_RENTALS_HOW_MANY_RENTS_SERVICE'))
	->orWhere('bhr.last_quarantine_date is null AND DATE_SUB(NOW(), INTERVAL '.sysConfig::get('EXTENSION_PAY_PER_RENTALS_HOW_MANY_MONTHS_SERVICE').' MONTH)<='.sysConfig::get('EXTENSION_PAY_PER_RENTALS_START_MAINTENANCE'))
	->orWhere('DATE_SUB(NOW(), INTERVAL '.sysConfig::get('EXTENSION_PAY_PER_RENTALS_HOW_MANY_MONTHS_SERVICE').' MONTH) <= bhr.last_quarantine_date')
	->orWhere('pib.status = ?','Q');
	...
	switch ($action){
		case 'edit':
			$infoBox->setForm(array(
					'action'    => itw_app_link(tep_get_all_get_params(array('action')) . 'action=save&type=4'),
					'method'    =>  'post',
			...
			$cancelButton = htmlBase::newElement('button')
				->usePreset('cancel')
				->setHref(itw_app_link(tep_get_all_get_params(array('action', 'appPage')), null, 'quarantine', 'SSL'));
		...
		default:
			if (isset($cInfo) && is_object($cInfo)) {
				...
				->setHref(itw_app_link(tep_get_all_get_params(array('action', 'mID')) . 'action=edit&mID=' . $cInfo->pay_per_rental_maintenance_id,'maintenance','quarantine'));
...
?>
 <div class="pageHeading"><?php echo sysLanguage::get('HEADING_TITLE_QUARANTINE');?></div>
 <br />
