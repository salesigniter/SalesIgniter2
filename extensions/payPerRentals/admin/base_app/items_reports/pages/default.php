...
$gridHeaderColumns = array(
	...
	array('text' => sysLanguage::get('TABLE_HEADING_CURRENT_MAINTENANCE_TYPE')),
	...
	foreach($products as $product){
		foreach($product['ProductsInventory'] as $inv){
			foreach($inv['ProductsInventoryBarcodes'] as $pib){
				...
				$QPayPerRentalMaintenancePeriods = Doctrine_Core::getTable('PayPerRentalMaintenancePeriods')->find($pib['BarcodeHistoryRented'][0]['current_maintenance_type']);
				if($QPayPerRentalMaintenancePeriods){
					$current_maintenance_type = $QPayPerRentalMaintenancePeriods->maintenance_period_name;
				}else{
					$current_maintenance_type = 'None';
				}
				...
				$gridBodyColumns = array(
					...
					array('text' => (isset($current_maintenance_type))?$current_maintenance_type:''),
					array('text' => $status),
