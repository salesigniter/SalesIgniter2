...
$(document).ready(function (){
	$('.returnButton').click(function(){
		...
		$.ajax({
			...
			url: js_app_link('appExt=payPerRentals&app=return_barcode&appPage=default&action=return'),
				...
				$.ajax({
					...
					success: function (data){
						$( '<div id="dialog-mesage" title="Return hire check">'+data.popupContent+'</div>' ).dialog({
							...
							height:700,
							close: function (e, ui){
								js_redirect(js_app_link('appExt=payPerRentals&app=return_barcode&appPage=default'));
