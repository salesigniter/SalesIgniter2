<?php
	...
	public function addTab(&$TabsObj, Product $Product) {
		...
		->where('pay_per_rental_id = ?', $PurchaseTypeCls->getPayPerRentalId())
		...
		if (count($Qperiods) > 0){
			...
			foreach($Qperiods as $iPeriod){
				...
				if ($Product->getId() > 0){
					...
					->andWhere('p.products_id=?', $Product->getId())
					->execute(array(), Doctrine_Core::HYDRATE_ARRAY);
		...
		->addTabPage('productPricingTab_' . $typeName, array('text' => ($pricingPeriods ? $pricingPeriods->draw() : '') . $pricingTable->draw()));
	}
}
