...
class OrderPaymentGateway2checkout extends StandardPaymentModule
{
	...
	public function __construct() {
		...
		$this->setFormUrl('https://www.2checkout.com/checkout/' . $this->getConfigData('PURCHASE_ROUTINE'));
	}
	...
	function getHiddenFields() {
		...
		if ($this->getConfigData('DEMO_MODE') == 'Y'){
		...
		$this->setParameter('action', 'remotePaymentProcess');
		...
		if ($this->getConfigData('PRODUCT_INFORMATION') == 'SalesIgniter'){
			...
			if ($onePageCheckout->isMembershipCheckout() === true){
				...
				foreach($order->products as $pInfo){
					...
					$this->setParameter('li_' . $i . '_price', $pInfo['final_price']);
				...
				while($CartProducts->valid() === true){
					...
					$this->setParameter('li_' . $i . '_price', $CartProduct->getFinalPrice());
	...
	function afterProcessPayment($success) {
		global $userAccount;
