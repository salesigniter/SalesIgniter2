...
$(document).ready(function (){
	...
	if ($.inArray($_GET, 'pID')){
		$('.gridBody > .gridBodyRow[data-plan_id="' + $_GET['pID'] + '"]').click();
	}
				...
				$(self).find('.saveButton').click(function (){
					...
					$.ajax({
						...
						success: function (data){
							if (data.success){
								js_redirect(js_app_link('appExt=' + thisAppExt + '&app=' + thisApp + '&appPage=' + thisAppPage + '&pID=' + data.pID));
							}else{
								alert(data.message);
							}
						}
	...
	$('.deleteButton').click(function (){
		...
		confirmDialog({
			confirmUrl: js_app_link('appExt=' + thisAppExt + '&app=' + thisApp + '&appPage=' + thisAppPage + '&action=delete&pID=' + planId),
