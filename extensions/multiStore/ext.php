...
class Extension_multiStore extends ExtensionBase
{
	...
	public function init()
	{
		...
		$Modules = Doctrine_Core::getTable('Modules')->getRecordInstance();

		$Module->hasMany('StoresModulesConfiguration as StoreConfig', array(
			'local' => 'modules_id',
			'foreign' => 'modules_id',
		...
		if (Session::exists('login_id')){
			...
			if (isset($_GET['stores_id'])){
				...
				foreach($_GET['stores_id'] as $storeId){
					...
					if (in_array($storeId, Session::get('admin_allowed_stores')) === false) {
						continue;
					}
					$validStores[] = $storeId;
				}
	...
	{
		$Qreviews
			->leftJoin('p.ProductsToStores p2s')
	...
	public function AdminHeaderRightAddContent()
	{
		...
		if ($Result){
			...
			$form->append(htmlBase::newElement('button')
				->setText('GO')
				->setType('submit'));
	...
	{
		$productQuery
			->leftJoin('p.SpecialsToStores s2s')
	...
	public function ProductInventoryReportsListingQueryBeforeExecute(&$Qproducts)
	{
		...
		if ($isInventory && $extInventoryCenters->stockMethod == 'Store'){
			if (!Session::exists('admin_showing_stores')){
				$Qproducts
					->leftJoin('pib.ProductsInventoryBarcodesToStores b2s')
	...
	public function ProductInventoryBarcodeHasInventoryQueryBeforeExecute($invData, &$Qcheck)
	{
		...
		if ($invData['use_store_inventory'] == '1'){
			$Qcheck
				->leftJoin('ib.ProductsInventoryBarcodesToStores b2s')
				->leftJoin('b2s.Stores');
			if ($appExtension->isAdmin()){
				if (is_object($Editor)){
	...
	public function ProductInventoryBarcodeGetInventoryItemsQueryBeforeExecute($invData, &$Qcheck)
	{
		...
		$Qcheck
			->leftJoin('ib.ProductsInventoryBarcodesToStores ib2s')
