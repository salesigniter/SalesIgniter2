...
class Extension_customFields extends ExtensionBase
{
	...
	{
		$contents['children']['productsBox']['children'][] = array(
	...
	public function ProductInfoBeforeDescription(&$product)
	{
		...
		if (!isset($productInfo['ProductsCustomFieldsToProducts']) || empty($productInfo['ProductsCustomFieldsToProducts']) || sizeof($productInfo['ProductsCustomFieldsToProducts']) <= 0) {
			return;
		}
	...
	public function CustomProductInfoGetFields(&$product)
	{
		...
		if ($productInfo['ProductsCustomFieldsToProducts'][0]['customFieldTotal'] <= 0) {
			return;
		}
				...
				if (!empty($fInfo['ProductsCustomFieldsToProducts'])){
					...
					$AdvancedTable->addBodyRow(array(
						'columns' => array(
							array(
								'addCls' => 'fieldKey',
								'text'   => $fInfo['ProductsCustomFieldsDescription'][0]['field_name']
							),
							array(
								'addCls' => 'fieldValue',
								'text'   => tep_draw_pull_down_menu($fInfo['search_key'], $dropArray)
							)
						)
					));
					unset($dropArray);
