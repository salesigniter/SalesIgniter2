<?php
class PurchaseTypeTabDownload_tab_pricing
{
	...
	public function addTab(&$TabsObj, Product $Product, $PurchaseType) {
		...
		if ($PurchaseType->getConfigData('PRICING_ENABLED') == 'True'){
			...
			$pricingTabs = htmlBase::newElement('tabs')
				->setId('purchaseTypeDownloadSettingsTabPricingTabs');
			...
			foreach($priceTabs as $tInfo){
				...
				$pricingTabs->addTabHeader('purchaseTypeDownloadSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $tInfo['name']))
					->addTabPage('purchaseTypeDownloadSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $globalTable . $inputTable->draw()));
			}

			$TabsObj->addTabHeader('purchaseTypeDownloadSettingsTabPricing', array('text' => $this->getHeading()))
				->addTabPage('purchaseTypeDownloadSettingsTabPricing', array('text' => $pricingTabs->draw()));
		}
	}
}
