<?php
class PurchaseTypeTabStream_tab_pricing
{
	...
	public function addTab(&$TabsObj, Product $Product, $PurchaseType) {
		...
		if ($PurchaseType->getConfigData('PRICING_ENABLED') == 'True'){
			...
			$pricingTabs = htmlBase::newElement('tabs')
				->setId('purchaseTypeStreamSettingsTabPricingTabs');
			...
			foreach($priceTabs as $tInfo){
				...
				$pricingTabs->addTabHeader('purchaseTypeStreamSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $tInfo['name']))
					->addTabPage('purchaseTypeStreamSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $globalTable . $inputTable->draw()));
			}

			$TabsObj->addTabHeader('purchaseTypeStreamSettingsTabPricing', array('text' => $this->getHeading()))
				->addTabPage('purchaseTypeStreamSettingsTabPricing', array('text' => $pricingTabs->draw()));
		}
	}
}
