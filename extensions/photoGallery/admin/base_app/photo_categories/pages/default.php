<?php
	...
	function addCategoryTreeToGrid($parentId, &$tableGrid, &$infoBoxes, $namePrefix = ''){
		...
		->orderBy('c.sort_order, cd.categories_title');

		EventManager::notify('PhotoGalleryCategoryListingQueryBeforeExecute', &$Qcategories);
		...
		if ($Result->count() > 0){
			foreach($Result->toArray(true) as $Category){
				...
				$infoBoxSettings = array(
					...
					'categoryName'         => $Category['PhotoGalleryCategoriesDescription'][Session::get('languages_id')]['categories_title'],
