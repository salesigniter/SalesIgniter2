...
class TemplateManagerWidgetArticles extends TemplateManagerWidget {
	...
	public function show(TemplateManagerLayoutBuilder $LayoutBuilder){
		...
		$Qtopics = Doctrine_Manager::getInstance()
			->getCurrentConnection()
			->fetchAssoc("select t.topics_id, td.topics_name, t.parent_id from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.parent_id = '0' and t.topics_id = td.topics_id and td.language_id = '" . (int)Session::get('languages_id') . "' order by sort_order, td.topics_name");
		foreach ($Qtopics as $topics)  {
		...
		if (isset($tPath) && tep_not_null($tPath)) {
			...
			while (list($key, $value) = each($tPath_array)) {
				...
				$Qtopics = Doctrine_Manager::getInstance()
					->getCurrentConnection()
					->fetchAssoc("select t.topics_id, td.topics_name, t.parent_id from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.parent_id = '" . (int)$value . "' and t.topics_id = td.topics_id and td.language_id = '" . (int)Session::get('languages_id') . "' order by sort_order, td.topics_name");
				if (sizeof($Qtopics) > 0) {
					$new_path .= $value;
