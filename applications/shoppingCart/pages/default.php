...
if ($ShoppingCart->hasContents()){
	...
	$shoppingCartHeader = array(
		...
		array(
			'addCls' => '',
			'text'   => sysLanguage::get('TABLE_HEADING_TOTAL'),
			'align'  => 'right'
		),
	);
...
<div class="main" style="text-align:right;">
	<span class="smallText" style="float:left;"></span><b><?php echo sysLanguage::get('SUB_TITLE_SUB_TOTAL'); ?> <?php echo $currencies->format($ShoppingCart->showTotal()); ?></b>
</div>
<div style="clear:both;"></div>
<?php
	if (sysConfig::exists('MODULE_SHIPPING_FREE_SHOW_TEXT')){
		/*Free shipping add*/
		require(sysConfig::getDirFsCatalog() . 'includes/modules/OrderShippingModules/freeshipping.php');
...
$pageContents = htmlBase::newElement('form')
	->setAction(itw_app_link('action=update_product', 'shoppingCart', 'default'))
