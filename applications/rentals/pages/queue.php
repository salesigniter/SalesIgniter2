...
if (!$userAccount->isLoggedIn()){
	...
	if(count($QPickupRequests) > 0){
		...
		foreach($QPickupRequests as $req){
			if(strtotime($req['start_date']) >= time()){
	...
	if ($shippedProducts){
		ob_start();
		echo '<div>'
			. (isset($updatePickup)?'<form name="updateRequest" method="post" action="'.itw_app_link(tep_get_all_get_params(array('action', 'products_id')) . 'action=updatePickupRequest').'">':'')
			. (isset($updatePickup)?$pickupReqCol->draw().'<br/>'. $updatePickup->draw().'</form><br/>':'')
			.'</div>';
