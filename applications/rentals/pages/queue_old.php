...
//here i check for parent
	if (!$userAccount->isLoggedIn()){
		...
		if(count($QPickupRequests) > 0){
			$QRentedQueueToPickupRequest = Doctrine_Query::create()
				->from('PickupRequests pr')
				->leftJoin('pr.PickupRequestsTypes prt')
				->leftJoin('pr.RentedProductsToPickupRequests rptpr')
				//->where('rptpr.rented_products_id = ?', $sInfo['customers_queue_id'])
				->execute(array(), Doctrine_Core::HYDRATE_ARRAY);
			...
			if(count($QRentedQueueToPickupRequest) > 0){
				$pickupReqCol->selectOptionByValue($QRentedQueueToPickupRequest[0]['pickup_requests_id']);
			}
		...
		if ($shippedProducts){
			ob_start();
			echo (isset($updatePickup)?'<form name="updateRequest" method="post" action="'.itw_app_link(tep_get_all_get_params(array('action', 'products_id')) . 'action=updatePickupRequest').'">':'') . '<div><b>' . sysLanguage::get('TEXT_SHIPPED_MOVIES') . ' - ' . $shippedProducts . '</b><br><br>';
			new productListingBox($info_box_contents);
			echo '</div><br/>' . (isset($updatePickup)?$pickupReqCol->draw().'<br/>'. $updatePickup->draw().'</form><br/>':'');
		...
		<tr>
			<td>
				...
				foreach($Qrental as $rInfo){
					...
					$purchaseTypeRental = PurchaseTypeModules::getModule('membershipRental');
					$purchaseTypeRental->loadProduct($productsId);
