...
$listing_sql = "select rp.rented_products_id, pd.products_name, p.products_image, p.products_id, rp.date_added  from products p
        INNER JOIN products_description pd ON pd.products_id = p.products_id
        INNER JOIN rented_products rp ON rp.products_id = p.products_id and customers_id = '" . $cid . "'
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td>
			<?php
			$products_count = sysConfig::get('MAX_DISPLAY_SEARCH_RESULTS');
				...
				while($listing = tep_db_fetch_array($listing_query)){
                  ...
                  where ptb.products_id=' . (int)$listing['products_id']);
					...
					if ($box_id){
						...
						if ($discs_count2 > $discs_count1) {
							$discs_count = $discs_count2;
						}
						else {
							$discs_count = $discs_count1;
						}
					}
					...
					$list_box_contents[$cur_row][] = array(
						...
						'text'   => '&nbsp;<a href="' . itw_app_link('products_id=' . $listing['products_id'], 'product', 'info') . '">' . tep_image($listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>&nbsp;'
					);
					$list_box_contents[$cur_row][] = array(
						'align'  => 'center',
						'params' => 'class="productListing-data"',
...
$pageContents = htmlBase::newElement('form')
	->setAction(itw_app_link(null, 'index', 'default', 'SSL'))
	->setName('frm')
	->setMethod('post')
	->html($pageContents)
	->draw();
