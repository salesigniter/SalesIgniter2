...
?>
                  <tr>
                    ...
                    <td class="smallText" align="right"><b><?php echo sysLanguage::get('HEADING_TOTAL'); ?></b></td>
					  <?php
					  //add event for heading Reservations
					$contents = EventManager::notifyWithReturn('OrderInfoProductTableHeading', $_GET['order_id']);
					if (!empty($contents)){
...
foreach($Order->getProducts() as $OrderProduct){
	...
	//addEvent for Reservations
	$contents = EventManager::notifyWithReturn('OrderInfoProductTableBody', $OrderProduct, $trackingCompanies);
					if (!empty($contents)){
						foreach($contents as $content){
							echo $content;
						}
...
<!-- Package Tracking Plus END -->
	<!--track reservation items individually -->
	<?php
...
<?php
	...
	$pageTitle = sysLanguage::get('HEADING_TITLE_HISTORY_INFO');
	
	$pageButtons = htmlBase::newElement('button')
	->usePreset('back')
	->setHref(itw_app_link(tep_get_all_get_params(array('order_id')), 'account', 'history', 'SSL'))
	->draw();
