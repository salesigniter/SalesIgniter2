...
function loadPageTwo() {
	...
	blockQueue();
	if ($('input[name=payment_method]:radio').size() == 0 && $('input[name=payment_method]:hidden').size() == 1){
		$('input[name=payment_method]:hidden').trigger('click');
	}
	else if ($('input[name=payment_method]:radio').size() == 1){
		$('input[name=payment_method]:radio').click();
	}
	else {
		$('input[name=payment_method]:radio').first().trigger('click');
	}

	if ($('input[name=shipping_method]').size() > 0){
		$.when(queueIsClear()).then(function () {
			blockQueue();

			if ($('input[name=shipping_method]:radio').size() == 0 && $('input[name=shipping_method]:hidden').size() == 1){
				$('input[name=shipping_method]:hidden').click();
			}
			else if ($('input[name=shipping_method]:radio').size() == 1){
				$('input[name=shipping_method]:radio').click();
			}
			else {
				$('input[name=shipping_method]:radio:eq(0)').click();
			}
		});
	}
	...
	if ($('.rentalPlans').length <= 0){
		//Is not needed, the click on all buttons will update the totals, first payment, then shipping
		//updateTotals();
	}
}
...
function queueIsClear() {
	if (deferredActive === false){
		return true;
	}
...
$(document).ready(function () {
	...
	$('input.quantity').live('change', function () {
		...
		$.ajax({
			...
			data : 'id=' + $(this).data('id') + '&' + $(this).attr('name') + '=' + $(this).val(),
			type : 'post',
			dataType : 'json',
			success : function (data) {
	...
	$('#continueButton').click(function () {
		...
		if ($('#currentPage').val() == 'success'){
			js_redirect(jsConfig.get('DIR_WS_CATALOG'));
			return false;
		}
		showAjaxLoader($('.checkoutContent'), 'xlarge', 'dialog');
		var linkParams = js_get_all_get_params(['app', 'appPage', 'action']);
		$.ajax({
			url : js_app_link(linkParams + 'rType=ajax&app=checkout&appPage=default&action=process&showErrors=true'),
			cache : false,
			dataType : 'json',
	...
	$('input[name=payment_method]').live('click', function (e) {
		e.stopPropagation();
	...
	$('input[name=shipping_method]').live('click', function (e) {
		e.stopPropagation();
