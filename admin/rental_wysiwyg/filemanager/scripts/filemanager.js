...
// Called when detail views are loaded.
	var bindToolbar = function (data) {
		...
		if (!has_capability(data, 'select')){
			...
			$('#fileinfo').find('button#select').click(
				function () {
					selectItem(data);
				}).show();
		}
		...
		if (!has_capability(data, 'rename')){
			...
			$('#fileinfo').find('button#rename').click(
				function () {
		...
		if (!has_capability(data, 'delete')){
			...
			$('#fileinfo').find('button#delete').click(
				function () {
					if (deleteItem(data)){
		...
		if (!has_capability(data, 'download')){
			...
			$('#fileinfo').find('button#download').click(
				function () {
	...
	var selectItem = function (data) {
		if (window.opener){
			...
			if ($.urlParam('CKEditor')){
				...
				$(window.opener.browserField).val(data['Path']);
			}
...
// successfully created.
	var addFolder = function (parent, name) {
		...
		if (parent != fileRoot){
			...
			$('#filetree').find('li a[rel="' + parent + name + '/"]').click(
				function () {
	...
	var getFileInfo = function (file) {
		...
		$.getJSON(fileConnector + '?mode=getinfo&path=' + file, function (data) {
			if (data['Code'] == 0){
				...
				if (data['Properties']['Size'] && data['Properties']['Size'] != ''){
...
// and list views with sorting options.
	var getFolderInfo = function (path) {
		...
		$.getJSON(url, function (data) {
			...
			if ($('#fileinfo').data('view') == 'grid'){
				$('#fileinfo').find('#contents li').click(
					function () {
