...
class Filemanager
{
		...
		if (!$handle = opendir($current_path)){
			...
			while(false !== ($file = readdir($handle))){
				if ($file != "." && $file != ".." && is_dir($current_path . $file)){
					...
					if ($file != "." && $file != ".." && !in_array($file, $this->config['unallowed_files'])){
						$this->item = array();
						$this->item['properties'] = $this->properties;
	...
	public function delete() {
		...
		if (is_dir($this->doc_root . $this->get['path'])){
			...
			if (file_exists($this->doc_root . $this->get['path'])){
	...
	private function get_file_info($path = '', $return = array()) {
		...
		if (is_dir($this->doc_root . $path)){
			...
			if (in_array(strtolower($this->item['filetype']), $this->config['images'])){
...
function makeWritable($filePath) {
	...
	if (!is_writable(sysConfig::getDirFsCatalog() . str_replace(sysConfig::getDirFsCatalog(), '', $filePath))){
		ftp_site($ftpConn, 'CHMOD 777, ' . str_replace(sysConfig::getDirFsCatalog(), '', $filePath));
		//error_log(str_replace(sysConfig::getDirFsCatalog(), '', $filePath) . "\n", 3, dirname(__FILE__) . '/error_log');
	}
}

function unmakeWritable($filePath) {
	global $ftpConn;
	if (substr($filePath, 0, 1) == DIRECTORY_SEPARATOR){
		$filePath = substr($filePath, 1);
	}
	...
	if (is_writable(sysConfig::getDirFsCatalog() . str_replace(sysConfig::getDirFsCatalog(), '', $filePath))){
		ftp_site($ftpConn, 'CHMOD 755, ' . str_replace(sysConfig::getDirFsCatalog(), '', $filePath));
		//error_log(str_replace(sysConfig::getDirFsCatalog(), '', $filePath) . "\n", 3, dirname(__FILE__) . '/error_log');
	}
}
...
function connectFtp() {
	...
	//error_log(($ftpCmd ? 'true' : 'false') . "\n", 3, dirname(__FILE__) . '/error_log');
	ftp_pasv($ftpConn, true);
	ftp_chdir($ftpConn, 'public_html');
