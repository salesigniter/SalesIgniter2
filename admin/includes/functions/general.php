...
function tep_cfg_select_new_product_tabs($value, $key = ''){
	$Files = array();
	$Dir = new DirectoryIterator(sysConfig::getDirFsAdmin() . 'applications/products/pages_tabs/');
	foreach($dir as $d){
		if ($d->isDot() || $d->isDir()) continue;

		$Files[] = str_replace(sysConfig::getDirFsCatalog(), '', $d->getPathname());
	}
	return tep_cfg_select_multioption_element($Files, $value, $key)->draw();
}
...
function tep_cfg_pull_down_zone_classes($zone_class_id, $key = '') {
	...
	return '<br>' . tep_draw_pull_down_menu($name, $zone_class_array, $zone_class_id);
}
...
function tep_cfg_pull_down_order_statuses($order_status_id, $key = '') {
	...
	return '<br>' . tep_draw_pull_down_menu($name, $statuses_array, $order_status_id);
}
