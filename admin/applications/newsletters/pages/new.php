...
<form name="newsletter" action="<?php echo itw_app_link((isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] . '&' : '') . 'action=save', 'newsletters', 'new');?>" method="post">
	<table border="0" cellspacing="0" cellpadding="2">
		...
		<tr>
			...
			<td class="main">
				<textarea name="content" style="width:100%" rows="20"><?php echo $Newsletter->content; ?></textarea>
			</td>
		</tr>
	</table>
		...
		<tr>
			<td class="main" align="right"><?php
				echo htmlBase::newElement('button')->usePreset('save')->setType('submit')->draw() . '&nbsp;&nbsp;' .
					htmlBase::newElement('button')->usePreset('cancel')
						->setHref(itw_app_link((isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] : ''), 'newsletters', 'default'))
						->draw();
				?></td>
		</tr>
	</table>
