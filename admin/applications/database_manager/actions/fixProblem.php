...
switch($Resolution){
	...
	case 'syncColumnSettings':
		...
		$message = 'Database table column settings syncronized.';
		$isOk = true;
		break;
	case 'removeColumn':
		Doctrine_Manager::getInstance()
	...
	case 'changeTableCollation':
		...
		//if ($success){
		$message = 'Database table collation changed to ' . $collation . '.';
		$isOk = true;
		$isCharset = true;
		//}else{
		//	$message = 'Error Changing Table Collation';
		//}
		break;
	case 'changeColumnCollation':
		$charset = $_GET['to'];
		$collation = $_GET['collate'];
		...
		if (!empty($EncodeInfo['encodeCols'])){
			$UpdateQueries = getEncodeUpdateQueries($tableName, $EncodeInfo);
		}
		...
		if ($success){
			$message = 'Database table column collation changed to ' . $collation . '.';
			$isOk = true;
			$isCharset = true;
		}
		else {
			$message = 'Error Changing Table Column Collation';
		}
		break;
	case 'changeVariable':
		...
		switch($variable){
			case 'system_character_set':
			case 'system_character_set_collation':
				$xmlObj = simplexml_load_file(
					sysConfig::getDirFsCatalog() . 'includes/configure.xml',
					'SimpleXMLExtended',
					LIBXML_NOCDATA
				);

				foreach($xmlObj->config as $cInfo){
					if (strtolower((string)$cInfo->key) == strtolower($variable)){
						$cInfo->value = $varValue;
						break;
					}
				}

				$success = $xmlObj->asPrettyXML(sysConfig::getDirFsCatalog() . 'includes/configure.xml');
				if ($success){
					$message = $variable . ' changed to ' . $varValue . '.';
					$isOk = true;
				}
				else {
					$message = 'Error Changing Variable: ' . $variable;
				}
				break;
			case 'character_set_database':
				$success = Doctrine_Manager::getInstance()
					->getCurrentConnection()
					->exec('alter database ' . sysConfig::get('DB_DATABASE') . ' default character set = ' . $varValue);
				if ($success){
					$message = 'Database character set changed to ' . $varValue . '.';
					$isOk = true;
					$isCharset = true;
				}
				else {
					$message = 'Error Changing Database Character Set';
				}
				break;
			case 'collation_database':
				$success = Doctrine_Manager::getInstance()
					->getCurrentConnection()
					->exec('alter database ' . sysConfig::get('DB_DATABASE') . ' default collate = ' . $varValue);
				if ($success){
					$message = 'Database collation changed to ' . $varValue . '.';
					$isOk = true;
					$isCharset = true;
				}
				else {
					$message = 'Error Changing Database Collation';
				}
				break;
			default:
...
/*$message = '<table>' .
	'<tr>' .
	'<td style="width:150px;text-align:left;line-height: 1.5em;vertical-align:top;"><b>Server Message:</b></td>' .
	'<td style="text-align:left;line-height: 1.5em;vertical-align:top;">' . $message . '</td>' .
	'</tr>' .
	(is_null($extName) === false ? '<tr>' .
		'<td style="width:150px;text-align:left;line-height: 1.5em;vertical-align:top;"><b>Extension Key:</b></td>' .
		'<td style="text-align:left;line-height: 1.5em;vertical-align:top;">' . $extName . '</td>' .
		'</tr>' : '') .
	(isset($tableName) ? '<tr>' .
