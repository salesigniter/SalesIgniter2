...
if (sysPermissions::isSimple() === false){
	...
	if ($orders){
		foreach($orders as $order){
			...
			$gridBodyColumns = array(
				...
				array(
					'text'  => $order['date_purchased']->format(sysLanguage::getDateTimeFormat()),
					'align' => 'center'
				),
				array(
	...
	$headerCols = array(
		array(
			...
			'valign' => 'bottom',
			'width'  => '45',
			'text'   => (sysLanguage::get('TABLE_HEADING_MONTH'))
		),
		array(
			'align'  => 'left',
			'valign' => 'bottom',
		...
		array(
			...
			'valign' => 'bottom',
			'width'  => '70',
			'text'   => (sysLanguage::get('TABLE_HEADING_INCOME'))
		),
		array(
			'align'  => 'right',
			'valign' => 'bottom',
		...
		array(
			...
			'valign' => 'bottom',
			'width'  => '70',
			'text'   => (sysLanguage::get('TABLE_HEADING_TAXED'))
		),
		array(
			'align'  => 'right',
			'valign' => 'bottom',
			'width'  => '70',
			'text'   => (sysLanguage::get('TABLE_HEADING_SHIPHNDL'))
		)
	);
	...
	if ($loworder){
		$headerCols[] = array(
			...
			'valign' => 'bottom',
			'width'  => '70',
			'text'   => (sysLanguage::get('TABLE_HEADING_LOWORDER'))
		);
	}
	...
	if ($extra_class){
		$headerCols[] = array(
			...
			'valign' => 'bottom',
			'width'  => '70',
			'text'   => (sysLanguage::get('TABLE_HEADING_OTHER'))
		);
	}
		...
		foreach($Result as $sInfo){
			...
			if ($rows > 1 && $sInfo['row_year'] <> $last_row_year){
				...
				$bodyRowCols = array(
					...
					array(
						'css'    => $cssRight,
						'format' => 'currency',
						'text'   => ($footer_sales_taxed)
					),
					array(
						'css'    => $cssRight,
						'format' => 'currency',
						'text'   => ($footer_shiphndl)
					)
				);
			...
			$bodyRowCols = array(
				...
				array(
					'align'  => 'right',
					'format' => 'currency',
					'text'   => ($sInfo['gross_sales'])
				),
				array(
					'align'  => 'right',
					'format' => 'currency',
					'text'   => ($net_sales_this_row + $zero_rated_net_sales_this_row)
				),
				array(
					'align'  => 'right',
					'format' => 'currency',
					'text'   => ($net_sales_this_row)
				),
				array(
					'align'  => 'right',
					'format' => 'currency',
					'text'   => ($shiphndl_this_row)
				)
			);
	...
	?>
<div id="desktopWidgets">
	<ul class="widget-column">
		<li class="ui-widget color-white ui-corner-all-big" id="favorites">
		...
		<li class="ui-widget color-white ui-corner-all-big" id="monthlySales">
			<div class="ui-widget-header ui-corner-top-big">
