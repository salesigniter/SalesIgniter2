					...
					<td valign="top">
						<table border="0" width="100%" cellspacing="0" cellpadding="2">
							...
							while($gv_list = tep_db_fetch_array($gv_query)){
								...
								<td class="dataTableContent" align="right"><?php if ((is_object($gInfo)) && ($gv_list['unique_id'] == $gInfo->unique_id)){
									echo tep_image(sysConfig::get('DIR_WS_TEMPLATE_IMAGES') . 'icon_arrow_right.gif');
								}
								else {
									echo '<a href="' . itw_app_link('page=' . $_GET['page'] . '&gid=' . $gv_list['unique_id'], 'coupons', 'gv_queue') . '">' . tep_image(sysConfig::get('DIR_WS_TEMPLATE_IMAGES') . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
								} ?>&nbsp;</td>
								</tr>
								<?php
							}
						...
						case 'release':
							...
							$contents[] = array(
								...
								'text'  => htmlBase::newElement('button')
									->setHref(itw_app_link('action=confirmrelease&gid=' . $gInfo->unique_id, 'coupons', 'gv_queue'))
									->setText('Confirm')->draw() . htmlBase::newElement('button')
									->setHref(itw_app_link('action=cancel&gid=' . $gInfo->unique_id, 'coupons', 'gv_queue'))
									->setText('Cancel')->draw()
							);
							break;
						default:
							...
							$contents[] = array(
								'align' => 'center',
								'text'  => htmlBase::newElement('button')
									->setHref(itw_app_link('action=release&gid=' . $gInfo->unique_id, 'coupons', 'gv_queue'))
									->setText('Release')->draw()
							);
							break;
					}
