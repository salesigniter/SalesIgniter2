...
if (isset($_GET['selectedCustomer']) && !empty($_GET['selectedCustomer'])){
    $dataExport = new dataExport();

	$fields = array();

	if (isset($_GET['v_customers_id'])){
		$fields[] = 'v_customers_id';
	}

	if (isset($_GET['v_customers_lastname'])){
		$fields[] = 'v_customers_lastname';
	}

	if (isset($_GET['v_customers_firstname'])){
		$fields[] = 'v_customers_firstname';
	}

	if (isset($_GET['v_customers_email_address'])){
		$fields[] = 'v_customers_email_address';
	}
	if (isset($_GET['v_customers_dob'])){
		$fields[] = 'v_customers_dob';
	}
	if (isset($_GET['v_customers_gender'])){
		$fields[] = 'v_customers_gender';
	}
	if (isset($_GET['v_customers_telephone'])){
		$fields[] = 'v_customers_telephone';
	}
	if (isset($_GET['v_customers_fax'])){
		$fields[] = 'v_customers_fax';
	}
	if (isset($_GET['v_customers_newsletter'])){
	...
	if (sizeof($fields) > 0) {
		...
		$QHeaders = Doctrine_Query::create()
					...
					->whereIn('ab.customers_id', explode(',', $_GET['selectedCustomer']))
		...
		if (count($QHeaders)){
			for($i=1;$i<=$maxVal;$i++){
				$fieldsp = array();
				if (isset($_GET['v_customers_addressbook_firstname'])){
					$fieldsp[] = 'v_customers_addressbook_firstname_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_lastname'])){
					$fieldsp[] = 'v_customers_addressbook_lastname_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_gender'])){
					$fieldsp[] = 'v_customers_addressbook_gender_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_company'])){
					$fieldsp[] = 'v_customers_addressbook_company_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_city'])){
					$fieldsp[] = 'v_customers_addressbook_city_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_state'])){
					$fieldsp[] = 'v_customers_addressbook_state_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_country'])){
					$fieldsp[] = 'v_customers_addressbook_country_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_address'])){
					$fieldsp[] = 'v_customers_addressbook_address_'.$i;
				}
				if (isset($_GET['v_customers_addressbook_postcode'])){
		...
		->whereIn('c.customers_id', $_GET['selectedCustomer']);
