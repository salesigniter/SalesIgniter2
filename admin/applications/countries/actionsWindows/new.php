...
if (isset($_GET['country_id'])){
	$Country = $Countries->find((int)$_GET['country_id']);
...
$listTable->addHeaderRow(array(
	...
	'columns' => array(
		array(
			'align' => 'left',
			'text'  => sysLanguage::get('TABLE_HEADING_ZONE_NAME')
		),
		array(
			'align' => 'left',
			'text'  => sysLanguage::get('TABLE_HEADING_ZONE_CODE')
		),
		array(
			'align' => 'right',
			'text'  => htmlBase::newElement('icon')->setType('insert')->addClass('insertIcon')
		)
	)
));
	...
	foreach($Zones as $zInfo){
		...
		$listTable->addBodyRow(array(
			'columns' => array(
				...
				array(
					'align' => 'right',
					'text'  => htmlBase::newElement('icon')->setType('delete')->addClass('deleteIcon')->draw()
				)
			)
		));
	}
