...
foreach(PurchaseTypeModules::getModules() as $purchaseType){
	...
	if ($Result){
		...
		foreach($Result as $inventory){
			...
			$track_method[$invController][$invType] = $invTrackMethod;
			...
			if ($Result){
				foreach($Result as $invItemInfo){
					...
					}elseif ($multiStoreInstalled === true){
						$centerId = $invItemInfo['inventory_store_id'];
					}
	...
	foreach($inventoryTypes as $typeId => $typeName){
		foreach($inventoryControllers as $controller => $cInfo){
			if (!isset($track_method[$controller])){
				$track_method[$controller] = array();
			}

			if (!isset($track_method[$controller][$typeId])){
				$track_method[$controller][$typeId] = '';
			}

			if (empty($track_method[$controller][$typeId])){
				$track_method[$controller][$typeId] = 'barcode';
			}
		}
	}
  ...
  <tr>
   <td><?php
   	foreach($inventoryControllers as $k => $v){
   		...
   		->setChecked($Product->getInventoryController() == $k)
   		->val($k)
