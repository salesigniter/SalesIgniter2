...
switch($params['orderBy']){
	case 'customer':
		$_orderBy = 'c.customers_firstname';
		break;
	case 'product':
		$_orderBy = 'pD.products_name';
		break;
	case 'date':
	default:
		$_orderBy = "sort_date";
	...
	foreach($rentalHistory as $rInfo){
		...
		$reportGrid->addBodyRow(array(
			'columns' => array(
				array(
					'align' => 'center',
					'text'  => $rInfo['Customers']['customers_firstname'] . ' ' . $rInfo['Customers']['customers_lastname']
				),
				array(
					'align' => 'center',
					'text'  => $rInfo['Customers']['AddressBook'][0]['entry_street_address']
				),
				array(
					'align' => 'center',
					'text'  => $rInfo['Products']['ProductsDescription'][1]['products_name']
				),
				array(
					'align' => 'center',
					'text'  => $rInfo['products_barcode']
				),
				array(
					'align' => 'center',
					'text'  => date('Y-m-d', strtotime($rInfo['sort_date']))
				),
				array(
					'align' => 'center',
					'text'  => $status
...
if ($params['filter'] != 'all'){
	$gridButtons[] = htmlBase::newElement('button')->setText('Show All')
		->setHref(itw_app_link(_buildUrl($_params), 'rental_history', 'default'));
}

$_params['filter'] = 'sent';
if ($params['filter'] != 'sent'){
	$gridButtons[] = htmlBase::newElement('button')->setText('Hide Returned')
		->setHref(itw_app_link(_buildUrl($_params), 'rental_history', 'default'));
}

$_params['filter'] = 'return';
if ($params['filter'] != 'return'){
	$gridButtons[] = htmlBase::newElement('button')->setText('Hide Sent')
		->setHref(itw_app_link(_buildUrl($_params), 'rental_history', 'default'));
}
