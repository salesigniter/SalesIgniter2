...
//ALTER TABLE  `admin` ADD  `admin_simple_admin` INT( 1 ) NOT NULL DEFAULT  '0'

//UPDATE  `abootoys_rental`.`products_inventory` SET  `type` =  'membershipRental' WHERE  `products_inventory`.`inventory_id` =355;
//UPDATE  `abootoys_rental`.`products_purchase_types` SET  `type_name` =  'membershipRental' WHERE  `products_purchase_types`.`purchase_type_id` =1;

/*update_products_to_standard only use when updating from SES1 to SES2*/

/*USE WITH CAUTION - better to uncomment one at a time. After update uncomment all*/

/*Doctrine_Query::create()
	->update('Products')
	->set('products_type', '?', 'standard')
	->execute();
*/

/*Doctrine_Query::create()
	->update('ProductsInventory')
	->set('type', '?', 'membershipRental')
	->where('type = ?', 'rental')
	->execute();
  */
/*
...
foreach($QProducts as $QProductsInventory){
	foreach($QProductsInventory['ProductsInventory'] as $pInv){
		if(count($pInv['ProductsInventoryBarcodes']) > 0 || count($pInv['ProductsInventoryQuantity']) > 0 && $pInv['ProductsInventoryQuantity'][0]['available'] > 0){
			$ProductsPurchaseType = new ProductsPurchaseTypes;
			$ProductsPurchaseType->products_id = $pInv['products_id'];
			$ProductsPurchaseType->status = '1';
			$ProductsPurchaseType->type_name = $pInv['type'];
			if($pInv['type'] == 'new'){
				$ProductsPurchaseType->price = $QProductsInventory['products_price'];
			}else{
				$ProductsPurchaseType->price = $QProductsInventory['products_price_used'];
			}
			$ProductsPurchaseType->tax_class_id = $QProductsInventory['products_tax_class_id'];
			$ProductsPurchaseType->inventory_track_method = $pInv['track_method'];
			$ProductsPurchaseType->inventory_controller = $pInv['controller'];
			$ProductsPurchaseType->save();
		}
	}
}
*/
