	...
	public function output() {
		...
		if ($orders){
			foreach($orders as $order){
				...
				$gridBodyColumns = array(
					...
					array('text'  => strip_tags($order['order_total']),
						  'align' => 'right',
						  'click' => 'document.location=\'' . $onClickLink . '\''
					),
					array('text'  => tep_datetime_short($order['date_purchased']),
						  'align' => 'center',
						  'click' => 'document.location=\'' . $onClickLink . '\''
					),
					array('text'  => $order['OrdersStatus']['OrdersStatusDescription'][Session::get('languages_id')]['orders_status_name'],
						  'align' => 'center',
						  'click' => 'document.location=\'' . $onClickLink . '\''
					)
				);
				...
				$gridBodyColumns[] = array('text'  => $arrowIcon->draw(),
										   'align' => 'right'
				);
		...
		switch($action){
			case 'delete':
				...
				$deleteButton = htmlBase::newElement('button')->setType('submit')->setName('delete')
					->usePreset('delete')->setText('Delete Order without Reservation and No Restock');
				$deleteButtonReservationRestock = htmlBase::newElement('button')->setType('submit')
					->setName('deleteReservationRestock')->usePreset('delete')
					->setText('Delete Order with Reservations and Restock of Reservation');
				$deleteReservationRestockAll = htmlBase::newElement('button')->setType('submit')
					->setName('deleteReservationRestockAll')->usePreset('delete')
					->setText('Delete Order with Restock All');
				$deleteButtonNoReservationRestock = htmlBase::newElement('button')->setType('submit')
					->setName('deleteRestockNoReservation')->usePreset('delete')
					->setText('Delete Order without Reservations and Restock');
				...
				$infoBox->addButton($deleteButton)->addButton($deleteButtonReservationRestock)
					->addButton($deleteButtonNoReservationRestock)->addButton($deleteReservationRestockAll)
			...
			default:
				if (isset($oInfo)){
					...
					$packingSlipButton = htmlBase::newElement('button')
						->setText(sysLanguage::get('TEXT_BUTTON_PACKINGSLIP'))
