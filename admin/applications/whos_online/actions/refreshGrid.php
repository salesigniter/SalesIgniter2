...
$tableGrid->addHeaderRow(array(
	'columns' => array(
		array(
			'text'    => sysLanguage::get('TABLE_HEADING_ONLINE'),
			'colspan' => 2,
			'align'   => 'center'
		),
		...
		array(
			'text'  => sysLanguage::get('TABLE_HEADING_USER_SESSION'),
			'align' => 'center'
		),
		array(
			'text'  => sysLanguage::get('TABLE_HEADING_HTTP_REFERER'),
			'align' => 'center'
		),
...
if ($WhosOnline){
	...
	foreach($WhosOnline as $oInfo){
		...
		// alteration for last url product name  eof
		$lastPageUrl = '<a HREF="' . ((sysConfig::get('REQUEST_TYPE') == 'SSL') ? sysConfig::get('HTTPS_SERVER') : sysConfig::get('HTTP_SERVER')) . $temp_url_link . '" target="_blank">' . $display_link . '</a>';
