...
foreach($Qcustomers as $iCustomer){
	...
	if ($rent_array[$position]['titles_to_send'] > $major) {
		$major = $rent_array[$position]['titles_to_send'];
	}
}
		...
		if ($qInfo['titles_to_send'] == $actual){
			$tableGrid->addBodyRow(array(
				'columns' => array(
					...
					array('text'  => $qInfo['movies_rented'],
						  'align' => 'center'
					),
					array('text'  => $qInfo['no_of_titles'],
						  'align' => 'center'
					),
					array('text'  => $qInfo['titles_to_send'],
						  'align' => 'center'
					),
					array('text'  => '<a href="' . itw_app_link(tep_get_all_get_params(array('action', 'cID', 'app', 'appPage')) . 'cID=' . $qInfo['customers_id'], 'rental_queue', 'details') . '">View</a>',
						  'align' => 'center'
					)
				)
			));
...
<div class="" align="left">
	<form name="filter" method="get" action="<?php echo itw_app_link(null, null, null);?>">
