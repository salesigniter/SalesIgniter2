	...
	default:
		if (isset($cInfo) && is_object($cInfo)){
			...
			$deleteButton = htmlBase::newElement('button')->setType('submit')->usePreset('delete')
				->setHref(itw_app_link(tep_get_all_get_params(array('action', 'arID')) . 'action=deleteConfirmAvail&arID=' . $cInfo->rental_availability_id));
...
<div style="width:75%;float:left;">
	...
	<div style="text-align:right;"><?php
		echo htmlBase::newElement('button')
			...
			->setHref(itw_app_link(tep_get_all_get_params(array(
				'action',
				'arID'
			)
		) . 'action=new'))->draw();
