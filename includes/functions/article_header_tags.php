...
function tep_get_header_tag_articles_keywords($article_id) {
  $article_header_tags = tep_db_query("select articles_head_keywords_tag from " . TABLE_ARTICLES_DESCRIPTION . " where language_id = '" . (int)Session::get('languages_id') . "' and articles_id = '" . (int)$_GET['articles_id'] . "'");
  $article_header_tags_values = tep_db_fetch_array($article_header_tags);
...
function tep_get_header_tag_articles_desc($article_id) {
  $article_header_tags = tep_db_query("select articles_head_desc_tag from " . TABLE_ARTICLES_DESCRIPTION . " where language_id = '" . (int)Session::get('languages_id') . "' and articles_id = '" . (int)$_GET['articles_id'] . "'");
  $article_header_tags_values = tep_db_fetch_array($article_header_tags);
