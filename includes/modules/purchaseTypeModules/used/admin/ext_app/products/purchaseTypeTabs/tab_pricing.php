<?php
class PurchaseTypeTabUsed_tab_pricing
{
	...
	public function addTab(&$TabsObj, Product $Product, $PurchaseType) {
		...
		if ($PurchaseType->getConfigData('PRICING_ENABLED') == 'True'){
			...
			$pricingTabs = htmlBase::newElement('tabs')
				->setId('purchaseTypeUsedSettingsTabPricingTabs');
			...
			foreach($priceTabs as $tInfo){
				...
				$pricingTabs->addTabHeader('purchaseTypeUsedSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $tInfo['name']))
					->addTabPage('purchaseTypeUsedSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $globalTable . $inputTable->draw()));
			}

			$TabsObj->addTabHeader('purchaseTypeUsedSettingsTabPricing', array('text' => $this->getHeading()))
				->addTabPage('purchaseTypeUsedSettingsTabPricing', array('text' => $pricingTabs->draw()));
		}
	}
}
