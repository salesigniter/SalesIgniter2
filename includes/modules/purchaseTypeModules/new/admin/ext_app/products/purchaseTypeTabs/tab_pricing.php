<?php
class PurchaseTypeTabNew_tab_pricing
{
	...
	public function addTab(&$TabsObj, Product $Product, $PurchaseType) {
		...
		if ($PurchaseType->getConfigData('PRICING_ENABLED') == 'True'){
			...
			$pricingTabs = htmlBase::newElement('tabs')
				->setId('purchaseTypeNewSettingsTabPricingTabs');
			...
			foreach($priceTabs as $tInfo){
				...
				$pricingTabs->addTabHeader('purchaseTypeNewSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $tInfo['name']))
					->addTabPage('purchaseTypeNewSettingsTabPricingTabs_' . $tInfo['id'], array('text' => $globalTable . $inputTable->draw()));
			}

			$TabsObj->addTabHeader('purchaseTypeNewSettingsTabPricing', array('text' => $this->getHeading()))
				->addTabPage('purchaseTypeNewSettingsTabPricing', array('text' => $pricingTabs->draw()));
		}
	}
}
