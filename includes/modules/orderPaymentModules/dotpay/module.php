	...
	function getHiddenFields() {
		...
		if (is_array($order->products)){
			...
			foreach($order->products as $pr => $ar){
				if (is_array($ar)){
					$my_order .= $ar['quantity'] . "x - " . $ar['name'] . " => " . $ar['model'] . ": " . $ar['final_price'] . " " . $order->info['currency'] . "\n";
				}
			}
		...
		//itwExit();
		$process_button_string = tep_draw_hidden_field('session_id', 'I' . '-' . $userAccount->getCustomerId() . '-' . $order->newOrder['orderID'] . '-' . ($onePageCheckout->isMembershipCheckout()
			? 'M' : '')) .
