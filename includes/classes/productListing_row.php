		...
		if ((isset($Pager) && $Pager->getNumResults() > 0) || (!isset($Pager) && sizeof($Products) > 0)){
			...
			if (sizeof($Products) > 0){
				foreach($Products as $pInfo){
					...
					if (method_exists($ProductType, 'canShowProductListing')){
						$canList = $ProductType->canShowProductListing();
					}
				...
				if (sysConfig::get('PRODUCT_LISTING_HIDE_NO_INVENTORY') == 'True'){
					if (sizeof($this->listedProductsIDS) > 0){
						...
						$this->templateData['pagerLinks'] = ($Pager->haveToPaginate()
							? $PagerLayout->display(array(), true) : '1 of 1');
					}
				}
			}
		}
		else {
			if ($this->showNoProducts === false) {
				return false;
			}
