...
class htmlWidget_fieldsetFormBlock extends htmlElement_fieldset
{
				...
				foreach($Row as $Field){
					...
					if (is_array($Field)){
						foreach($Field as $RowField){
							$this->addFieldToColumn($InputColumn, $RowField, $numberOfFields);
						}
					}
					else {
						$this->addFieldToColumn($InputColumn, $Field, $numberOfFields);
					}
	...
	private function addFieldToColumn(&$Column, &$Field, $RowFieldCount)
	{
		if ($Field->attr('type') != 'radio' && $Field->attr('type') != 'checkbox' && $Field->getTagName() != 'div'){
			if ($RowFieldCount == 1){
				$Field->css('width', '99%');
			}
			else {
