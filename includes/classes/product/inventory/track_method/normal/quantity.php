...
class productInventoryNormal_quantity {
	...
	public function getNextInventoryItemId(){
		$QinvItem = Doctrine_Query::create()
			->select('iq.quantity_id')
			->from('ProductsInventoryQuantity iq')
			->where('iq.inventory_id = ?', $this->invData['inventory_id'])
			->limit('1');
			
		EventManager::notify('ProductInventoryQuantityGetNextInventoryItemIdQueryBeforeExecute', $this->invData, &$QinvItem);
			
		$Result = $QinvItem->execute()->toArray();
		return $Result[0]['quantity_id'];
	}
