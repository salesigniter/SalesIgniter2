...
//  $coupon_query = tep_db_query("select * from " . TABLE_COUPONS . " where coupon_id = '" . $_GET['cID'] . "'");
  ...
  $coupon_desc_query = tep_db_query("select * from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $_GET['cID'] . "' and language_id = '" . Session::get('languages_id') . "'");
  ...
  for ($i = 0; $i < count($cat_ids); $i++) {
    $result = tep_db_query("SELECT * FROM categories, categories_description WHERE categories.categories_id = categories_description.categories_id and categories_description.language_id = '" . Session::get('languages_id') . "' and categories.categories_id='" . $cat_ids[$i] . "'");
  ...
  for ($i = 0; $i < count($pr_ids); $i++) {
    $result = tep_db_query("SELECT * FROM products, products_description WHERE products.products_id = products_description.products_id and products_description.language_id = '" . Session::get('languages_id') . "'and products.products_id = '" . $pr_ids[$i] . "'");
    if ($row = tep_db_fetch_array($result)) {
...
<p class="smallText" align="right"><?php echo '<a href="javascript:window.close()">' . sysLanguage::get('TEXT_CLOSE_WINDOW') . '</a>'; ?></p>
