...
if ($ImageCache->loadData() === true){
	...
	if (!empty($_GET['imgSrc'])){
		...
		if (file_exists($_GET['imgSrc'])){
			...
			if ($maxWidth !== null && $img->getimagewidth() > $maxWidth){
				$_GET['width'] = $maxWidth;
			}
			//$img->trimImage(.5);
			...
			if (isset($_GET['width']) && isset($_GET['height'])){
				...
				if (isset($_GET['width'])){
					$img->resizeimage($_GET['width'],0,Imagick::FILTER_LANCZOS,true);
				}elseif (isset($_GET['height'])){
					$img->resizeimage(0,$_GET['height'],Imagick::FILTER_LANCZOS,true);
				}
			}
